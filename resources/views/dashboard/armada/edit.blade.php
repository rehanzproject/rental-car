<form action="{{ route('armada.edit', ['armada' => $armada->id]) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="modal fade" id="editArmadaModal{{ $armada->id }}" tabindex="-1" aria-labelledby="editArmadaModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="input-group">
                        ...
                        <div class="mt-1">
                            <input name="mobilImages" class="form-control @error('mobilImages') is-invalid @enderror" value="{{ old('mobilImages', $armada->mobilImages) }}" type="file" id="file{{ $armada->id }}" accept="image/*" onchange="loadFile{{ $armada->id }}(event, {{ $armada->id }})">
                        </div>
                    </div>
                    @error('mobilImages')
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('typemobil_id') is-invalid @enderror" name="typemobil_id" id="mobil">
                            @forelse ($typemobils as $typemobil)
                                <option value="{{ $typemobil->id }}" {{ $armada->typemobil_id == $typemobil->id ? 'selected' : '' }}>{{ $typemobil->type_mobil }}</option>
                            @empty
                                <span>Belum ada Type Mobil, Silahkan membuat Type Mobil terlebih dahulu!</span>
                            @endforelse
                        </select>
                        @error('mobil')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('nama_mobil') is-invalid @enderror" id="nama_mobil" name="nama_mobil" value="{{ old('nama_mobil', $armada->nama_mobil) }}" required>
                        @error('nama_mobil')
                    ...
                    <div class="form-group">
                        ...
                        <div class="btn-group" role="group" aria-label="Basic radio toggle button group">
                            ...
                            {{-- Disewa --}}
                            <input type="radio" class="btn-check" name="status" id="status2{{ $armada->id }}" value="1" {{ $armada->status == 1 ? 'checked' : '' }} autocomplete="off">
